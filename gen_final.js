...
const TOOLS = [
  ...
  { name: 'Next.js',    slug: 'nextdotjs',  url: 'https://nextjs.org',              color: '#E8E8E8' },
  ...
  { name: 'GitHub',     slug: 'github',     url: 'https://github.com',              color: '#E8E8E8' },
...
for (const t of TOOLS) {
  if (NO_ICON[t.slug]) {
    output += `  ${t.name}: null /* hasBrandBadge:true */\n`;
    continue;
  }
  let raw = data[t.slug];
  ...
  output += `  ${t.name}: { color: '${color}', inner: '${esc(inner)}' },\n`;
}
fs.writeFileSync('tmp_final_icons.ts', output);
console.log('Done — written to tmp_final_icons.ts');
