...
float digit(vec2 p){
  ...
  if(uUseMouse > 0.5){
    vec2 mw = uMouse * uScale; float d = distance(s, mw);
    float inf = exp(-d * 8.0) * uMouseStrength * 10.0; intensity += inf;
    intensity += sin(d * 20.0 - iTime * 5.0) * 0.1 * inf;
  }
  if(uUsePageLoadAnimation > 0.5){
    float cr = fract(sin(dot(s, vec2(12.9898, 78.233))) * 43758.5453);
    float pd = clamp((uPageLoadProgress - cr * 0.8) / 0.2, 0.0, 1.0);
    intensity *= smoothstep(0.0, 1.0, pd);
  }
...
export default function FaultyTerminal({
  ...
  const timeOffRef = useRef<number>(Math.random() * 100);

  const tintVec = useMemo(() => hexToRgb(tint), [tint]);
  const ditherVal = useMemo(
    () => (typeof dither === "boolean" ? (dither ? 1 : 0) : dither as number),
    [dither]
  );
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      const c = rendererRef.current.gl;
      prog.uniforms.iResolution.value = new Color(c.canvas.width, c.canvas.height, c.canvas.width / c.canvas.height);
    }
    ...
    function tick(t: number) {
      ...
      const elapsed = (t * 0.001 + timeOffRef.current) * timeScale;
      prog.uniforms.iTime.value = elapsed;

      if (pageLoadAnimation && loadStartRef.current > 0) {
        prog.uniforms.uPageLoadProgress.value = Math.min((t - loadStartRef.current) / 2000, 1);
      }
      ...
      if (mouseReact) {
        ...
        sm.x += (m.x - sm.x) * df;
        sm.y += (m.y - sm.y) * df;
        prog.uniforms.uMouse.value = new Float32Array([sm.x, sm.y]);
      }
    ...
    if (gl.canvas.parentElement !== ctn) ctn.appendChild(gl.canvas);
    if (mouseReact) ctn.addEventListener("mousemove", handleMouse);
    ...
    return () => {
      ...
      timeOffRef.current = Math.random() * 100;
    };
  }, [
    scale, gridMul, digitSize, timeScale, pause, scanlineIntensity,
    glitchAmount, flickerAmount, noiseAmp, chromaticAberration,
    ditherVal, curvature, tintVec, mouseReact, mouseStrength,
    pageLoadAnimation, brightness, handleMouse,
  ]);
