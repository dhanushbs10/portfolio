...
8. **Projects** — top 3–4 featured, "View all projects" → `/projects`.
...
Set up Drizzle + Supabase (or start with typed local JSON and migrate later — recommend JSON first so you're not blocked on DB setup before there's anything to show), define all schemas from §5–7, seed with real content.
